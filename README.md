...
1. 下载 `SuperBrain-Lite-0.2.6-win-x64.zip`，解压到可写文件夹，例如 `D:\super-brain`。
...
运行文件约 **129 KB**，依赖 Windows 自带的 **.NET Framework 4.8 或更高版本**，面向 Windows 10/11 x64。没有捆绑浏览器运行环境。此体积指下载的运行文件，不代表进程内存占用。
...
资料不再存放到 C 盘 AppData。软件没有安装器。开启后台启动时注册当前用户的 Windows 登录任务，可在设置中关闭；无需额外常驻服务，不修改 Windows 全局设置。升级旧版后，勾选后台启动并保存设置会把旧 Run 启动项迁移为登录任务。移动或重命名运行文件后，请在新位置重新启用后台启动。若快捷键被占用，在设置中换一个即可。
