...
1. 下载 `SuperBrain-Lite-0.2.2-win-x64.zip`，解压到可写文件夹，例如 `D:\super-brain`。
...
运行文件约 **100 KB**，依赖 Windows 自带的 **.NET Framework 4.8 或更高版本**，面向 Windows 10/11 x64。没有捆绑浏览器运行环境。此体积指下载的运行文件，不代表进程内存占用。
